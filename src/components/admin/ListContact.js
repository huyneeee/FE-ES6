    ...
    async render() {
        ...
        ${contact.map((c,index) =>{
            return `
                ...
                <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-no-wrap p-4">
                <button href="/#/contact/${c._id}" class="btn btn-remove btn-primary btn-remove px-3 py-2 bg-red-500 rounded text-white" data-id="${c._id}">Delete</button>
                </td>
            </tr>
            `
        }).join("")}
    ...
    async afterRender() {
      ...
      btns.forEach( btn => {
          ...
          btn.addEventListener('click',async function(){
              ...
              if(question){
                try{
                  await ContactApi.remove(id,userId);
