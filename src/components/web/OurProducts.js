...
const OurProducts = {
    async render() {
        ...
        const { data : products } = await ProductApi.getProductsByCateId(1);
        ...
        return `
            ...
            <li>
               ${category.map(cate => {
                    return `
                            <button data-id="${cate.id}" class="tablinks mr-10 pb-2 hover:border-b-2 hover:border-black  focus:outline-none hover:text-gray-700  rounded-none ${cate.id==1?'border-b border-gray-500  font-semibold':''}">
                            ${cate.name}</button>
                   `
                }).join("")}
